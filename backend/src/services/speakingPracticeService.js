...
export async function analyzeAllRoundsAndSummary(sessionId) {
  ...
  // Phân tích từng round chưa được phân tích
  for (const round of rounds.rows) {
    // Nếu đã có analysis, bỏ qua
    if (round.analysis && round.score > 0) {
      continue;
    }
    ...
    if (!fs.existsSync(localPath)) {
      console.warn(`⚠️ Audio file not found: ${localPath}`);
      continue;
    }
    ...
    if (transcript) {
      ...
      try {
        analysis = await analyzePronunciation(transcriptText, round.prompt, level);
        score = analysis.score || 0;
        feedback = analysis.feedback || "";
        errors = analysis.errors || [];
        correctedText = analysis.corrected_text || "";
