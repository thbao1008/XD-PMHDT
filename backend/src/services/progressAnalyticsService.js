// backend/src/services/progressAnalyticsService.js
import pool from "../config/db.js";
import * as assistantAIService from "./assistantAIService.js";
 ...
 */
export async function getProgressAnalytics(learnerId) {
  try {
    ...
    // 4. AI Recommendations (sử dụng AiESP để đánh giá và đưa ra gợi ý)
...
/**
 * Generate AI recommendations sử dụng AiESP
 */
async function generateAIRecommendations(learnerId, speakingStats, challengeStats, overallProgress) {
  try {
...
Trả về JSON:
{
  ...
  "priority_areas": ["string"] // Các lĩnh vực cần ưu tiên
    ...
    // Gọi AiESP để phân tích
    const aiResponse = await assistantAIService.generateConversationResponse(
      analysisPrompt,
      []
    );
    ...
    if (aiResponse) {
      try {
        ...
        if (jsonMatch) {
          ...
          recommendations.overall_assessment = aiResponse.substring(0, 200);
        }
      } catch (e) {
        // Fallback: use text as assessment
        recommendations.overall_assessment = aiResponse.substring(0, 300);
      }
    }
...
export async function learnFromChallengeEvaluation(
  ...
  try {
    // Lưu vào assistant_ai_training để AiESP học cách đánh giá challenge
    await pool.query(
    ...
    // Cũng gọi mentorAiService để học từ mentor feedback
    if (feedback) {
      const mentorAiService = await import("./mentorAiService.js");
      await mentorAiService.learnFromMentorFeedback(
        feedback,
        scores,
        {
          learner_id: learnerId,
