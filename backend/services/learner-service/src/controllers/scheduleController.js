// Learner Service - Schedule Controller
...
export async function getLearnerSchedules(req, res) {
  try {
    ...
    // Kiểm tra learner có mentor không
    ...
    if (!learnerCheck.rows[0]?.mentor_id) {
      return res.json([]); // Trả về mảng rỗng nếu không có mentor
    }
...
/**
 * Lấy tất cả lịch học của mentor (tất cả learners của mentor)
 */
...
export async function createSchedule(req, res) {
  try {
    ...
    // Kiểm tra learner có thuộc mentor này không
    const learnerCheck = await pool.query(
    ...
    // Kiểm tra giờ trong khoảng 8-21h
    ...
    // Kiểm tra không trùng khung giờ trong cùng ngày
    ...
    // Kiểm tra chỉ được nhập cho tuần tiếp theo
